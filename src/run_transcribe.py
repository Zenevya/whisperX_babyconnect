#!/usr/bin/env python3
"""Transcription runner (moved to src/ for clearer project layout).
This is a copy of the workspace runner; use the CLI to run transcriptions.
"""
import argparse
import os
import sys
...
def run_transcription(args):
    ...
    print(f"Audio load + preprocess time: {tb-ta:.2f}s, samples={getattr(audio,'shape',None)} sr={sample_rate}")

    print("Transcribing...")
    tt0 = time.perf_counter()
    result = model.transcribe(audio, chunk_size=args.chunk_size)
    tt1 = time.perf_counter()
    print(f"Transcription time: {tt1-tt0:.2f}s")
